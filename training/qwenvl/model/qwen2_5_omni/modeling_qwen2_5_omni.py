    ...
    def get_rope_index(
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                # Safety check: align llm_positions with attention_mask to avoid shape mismatch
                valid_mask = attention_mask[i] == 1
                num_valid = valid_mask.sum().item()
                num_positions = llm_positions.shape[1]
                if num_positions != num_valid:
                    logger.warning(
                        f"[get_rope_index] Shape mismatch for sample {i}: "
                        f"llm_positions has {num_positions} but attention_mask has {num_valid} valid tokens. "
                        f"Truncating/padding to align."
                    )
                    if num_positions < num_valid:
                        # Pad llm_positions with the last position id + increments
                        last_pos = llm_positions[:, -1:]
                        pad = last_pos + torch.arange(1, num_valid - num_positions + 1, device=llm_positions.device).unsqueeze(0)
                        llm_positions = torch.cat([llm_positions, pad], dim=1)
                    else:
                        # Truncate llm_positions
                        llm_positions = llm_positions[:, :num_valid]
                position_ids[..., i, valid_mask] = llm_positions.to(position_ids.device)
...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig):
        ...
        if getattr(config, "train_classify", False):
            ...
            self.use_siglip_loss = False
            self.use_tuple_infonce = getattr(config, "use_tuple_infonce", False)
            self.tuple_aug_weight = getattr(config, "tuple_aug_weight", 1.0)
            self.tuple_single_weight = getattr(config, "tuple_single_weight", 1.0)
            self.tuple_bi_weight = getattr(config, "tuple_bi_weight", 1.0)
            self.hn_audio_only = getattr(config, "hn_audio_only", False)

            # TupleInfoNCE enforces all_layer: media → All Layers → Fusion Module
            if self.use_tuple_infonce and self.classify_type != "all_layer":
                print(f"[TupleInfoNCE] Overriding classify_type from '{self.classify_type}' to 'all_layer' to match architecture")
            ...
            if self.use_tuple_infonce:
                print(f"TupleInfoNCE enabled: aug_w={self.tuple_aug_weight}, single_w={self.tuple_single_weight}, bi_w={self.tuple_bi_weight}, hn_audio_only={self.hn_audio_only}")
    ...
    def _process_audio_features(self, input_features, feature_attention_mask, input_raw_wav, device):
        """Process raw audio through audio tower (+ BEATs if enabled).
        Returns: (audio_features, audio_feature_lengths)
        """
        ...
        audio_outputs = self.audio_tower(input_features, feature_lens=audio_feature_lengths, aftercnn_lens=audio_feat_lengths)
        ...
        if self.use_beats:
            ...
            for raw_wav in input_raw_wav:
                if raw_wav is None:
                    raw_wav = torch.zeros(16000, dtype=torch.float32, device=device)
            ...
            beats_proj = self.beats_ln(pool_features)
            beats_proj = self.beats_proj(beats_proj)

            if self.beats_only:
                audio_features = beats_proj
            else:
                audio_features = audio_features.unsqueeze(1)
                beats_proj = beats_proj.unsqueeze(1).to(audio_features.dtype)
                audio_features = torch.cat([audio_features, beats_proj], dim=1).view(-1, audio_features.size(-1))

        return audio_features, audio_feature_lengths

    def _extract_embedding(self, outputs):
        """Extract embedding from model outputs using classify_linear."""
        if self.classify_type == "last_layer":
            embed = outputs[0][:, -1, :]
            embed = self.classify_linear(embed)
        elif self.classify_type == "all_layer":
            all_hidden = outputs.hidden_states[1:]
            all_last = [h[:, -1, :] for h in all_hidden]
            embed = torch.cat(all_last, dim=-1)
            embed = self.classify_linear(embed)
        else:
            raise NotImplementedError
        return embed

    def _forward_submodal_embedding(self, sub_input_ids, cached_audio_features,
                                     cached_video_embeds, video_grid_thw,
                                     audio_feature_lengths, video_second_per_grid,
                                     has_audio, has_video, output_hidden_states):
        """Forward pass for sub-modal inputs, reusing cached audio/video features."""
        inputs_embeds = self.get_input_embeddings()(sub_input_ids)
        attention_mask = sub_input_ids.ne(PAD_TOKEN_ID).to(torch.int64)

        if has_audio and cached_audio_features is not None:
            audio_mask = (sub_input_ids == self.config.audio_token_id).unsqueeze(-1).expand_as(inputs_embeds).to(inputs_embeds.device)
            inputs_embeds = inputs_embeds.masked_scatter(audio_mask, cached_audio_features.to(inputs_embeds.device, inputs_embeds.dtype))

        if has_video and cached_video_embeds is not None:
            video_mask = (sub_input_ids == self.config.video_token_id).unsqueeze(-1).expand_as(inputs_embeds).to(inputs_embeds.device)
            inputs_embeds = inputs_embeds.masked_scatter(video_mask, cached_video_embeds.to(inputs_embeds.device, inputs_embeds.dtype))

        use_audio_in_video = has_audio and has_video
        position_ids, _ = self.get_rope_index(
            sub_input_ids, None,
            video_grid_thw if has_video else None,
            attention_mask,
            use_audio_in_video,
            audio_feature_lengths if has_audio else None,
            video_second_per_grid if has_video else None,
        )
    ...
    def _compute_symmetric_infonce(self, embeds_a, embeds_b, temperature):
        """Compute symmetric InfoNCE loss between two sets of embeddings with all_gather."""
        return self._compute_infonce_with_hard_negatives(embeds_a, embeds_b, [], temperature)

    def _compute_infonce_with_hard_negatives(self, embeds_a, embeds_b, hard_negs, temperature):
        """Symmetric InfoNCE with optional hard negatives appended to the negative pool.

        Hard negatives are LOCAL only (no all_gather) to avoid NCCL deadlock when
        different ranks have different numbers of hard negatives.

        Args:
            embeds_a: [B, D] anchor embeddings
            embeds_b: [B, D] positive embeddings
            hard_negs: list of [B, D] tensors — hard negative embeddings (local, no_grad)
            temperature: temperature scalar
        """
        # Fixed 2 all_gather calls (same across all ranks)
        all_a = torch.stack(dist.nn.all_gather(embeds_a), dim=0)
        all_b = torch.stack(dist.nn.all_gather(embeds_b), dim=0)

        gpu_num, batch_size = all_a.shape[0], all_a.shape[1]
        all_a_flat = F.normalize(all_a, dim=-1).transpose(0, 1).reshape(-1, all_a.size(-1))
        all_b_flat = F.normalize(all_b, dim=-1).transpose(0, 1).reshape(-1, all_b.size(-1))

        # Hard negatives: local only, no all_gather
        local_neg_cat = None
        if hard_negs:
            local_neg_parts = [F.normalize(neg, dim=-1) for neg in hard_negs]
            local_neg_cat = torch.cat(local_neg_parts, dim=0)  # [K*B, D]

        a_norm = F.normalize(embeds_a, dim=-1)
        b_norm = F.normalize(embeds_b, dim=-1)

        rank_labels = torch.tensor(
            [dist.get_rank() + i * gpu_num for i in range(batch_size)],
            device=embeds_a.device, dtype=torch.int64
        )
        loss_fn = LigerCrossEntropyLoss()

        # a → b: positives (global) + hard negatives (local)
        sims_a2b = torch.matmul(a_norm, all_b_flat.T) / temperature
        if local_neg_cat is not None:
            sims_a2neg = torch.matmul(a_norm, local_neg_cat.T) / temperature
            sims_a2b = torch.cat([sims_a2b, sims_a2neg], dim=1)
        loss_a2b = loss_fn(sims_a2b, rank_labels)

        # b → a: positives (global) + hard negatives (local)
        sims_b2a = torch.matmul(b_norm, all_a_flat.T) / temperature
        if local_neg_cat is not None:
            sims_b2neg = torch.matmul(b_norm, local_neg_cat.T) / temperature
            sims_b2a = torch.cat([sims_b2a, sims_b2neg], dim=1)
        loss_b2a = loss_fn(sims_b2a, rank_labels)
    ...
    def forward(
        ...
        input_raw_wav = None,
        # TupleInfoNCE parameters
        aug_pixel_values_videos = None,
        aug_video_grid_thw = None,
        aug_video_second_per_grid = None,
        aug_input_features = None,
        aug_feature_attention_mask = None,
        aug_input_raw_wav = None,
        tuple_pos_input_ids = None,
        neg_text_input_ids = None,
        audio_only_input_ids = None,
        video_only_input_ids = None,
        ta_input_ids = None,
        tv_input_ids = None,
        av_input_ids = None,
        **kwargs
        ...
        _cached_audio_features = None
        _cached_video_embeds = None
        if input_ids is not None and input_ids.shape[1] != 1:  # Prefill stage
            if input_features is not None:
                ...
                if self.use_beats:
                    ...
                    for raw_wav in input_raw_wav:
                        if raw_wav is None:
                            # Generate 1s silence for samples without audio
                            raw_wav = torch.zeros(16000, dtype=torch.float32, device=input_ids.device)
                        for k in range(0, len(raw_wav), 16000 * 300):
                            raw_wav_k = raw_wav[k: k + 16000 * 300] 
                            if len(raw_wav_k) < 16000:
                                raw_wav_k = torch.cat([raw_wav_k, torch.zeros([16000 - len(raw_wav_k)], dtype=raw_wav_k.dtype, device=raw_wav_k.device)])
                ...
                # Cache audio features for TupleInfoNCE before scattering
                _cached_audio_features = audio_features
            ...
            if pixel_values_videos is not None:
                ...
                # Cache video embeds for TupleInfoNCE before scattering
                _cached_video_embeds = video_embeds
                video_mask = (
        ...
        if not self.train_classify:
            ...
            if all_ids is None or (self.training and all([it != "give_neg" for it in types])):
                ...
                # ===================== TupleInfoNCE Branch =====================
                # Always enter if use_tuple_infonce is True (all ranks must execute the same
                # number of all_gather calls to avoid NCCL deadlock)
                if getattr(self, 'use_tuple_infonce', False) and self.training:
                    anchor_embed = mllm_embeds  # full-modal (A+V+T) MLLM embedding
                    _has_aug = aug_pixel_values_videos is not None

                    # --- Objective 1: Full-modal augmented contrastive ---
                    if _has_aug:
                        # Augmented video features
                        aug_video_embeds = self.visual(aug_pixel_values_videos.type(self.visual.dtype), grid_thw=aug_video_grid_thw)

                        # Augmented audio features (if available, otherwise reuse original)
                        if aug_input_features is not None and aug_feature_attention_mask is not None:
                            _aug_audio_features, _ = self._process_audio_features(
                                aug_input_features, aug_feature_attention_mask, aug_input_raw_wav, input_ids.device
                            )
                        else:
                            _aug_audio_features = _cached_audio_features

                        # Build positive pair: tuple_pos_input_ids has aug_caption
                        pos_ids = tuple_pos_input_ids if tuple_pos_input_ids is not None else input_ids
                        aug_inputs_embeds = self.get_input_embeddings()(pos_ids)
                        if _aug_audio_features is not None:
                            aug_audio_mask = (pos_ids == self.config.audio_token_id).unsqueeze(-1).expand_as(aug_inputs_embeds).to(aug_inputs_embeds.device)
                            aug_inputs_embeds = aug_inputs_embeds.masked_scatter(aug_audio_mask, _aug_audio_features.to(aug_inputs_embeds.device, aug_inputs_embeds.dtype))
                        aug_video_mask = (pos_ids == self.config.video_token_id).unsqueeze(-1).expand_as(aug_inputs_embeds).to(aug_inputs_embeds.device)
                        aug_inputs_embeds = aug_inputs_embeds.masked_scatter(aug_video_mask, aug_video_embeds.to(aug_inputs_embeds.device, aug_inputs_embeds.dtype))

                        pos_attn = pos_ids.ne(PAD_TOKEN_ID).to(torch.int64).to(aug_inputs_embeds.device)
                        aug_position_ids, _ = self.get_rope_index(
                            pos_ids, image_grid_thw, aug_video_grid_thw, pos_attn,
                            use_audio_in_video, audio_feature_lengths, aug_video_second_per_grid,
                        )
                        aug_outputs = self.model(
                            attention_mask=pos_attn, position_ids=aug_position_ids,
                            inputs_embeds=aug_inputs_embeds,
                            output_hidden_states=output_hidden_states, return_dict=True,
                        )
                        pos_embed = self._extract_embedding(aug_outputs)

                        # Construct hard negative (no grad, deterministic selection)
                        hard_neg_embeds = []
                        batch_size_local = input_ids.size(0)
                        neg_step = self.cache_dict.get('_neg_step', 0)
                        self.cache_dict['_neg_step'] = neg_step + 1
                        # Default: cycle text(0) → audio(1) → video(2).
                        # When config.hn_audio_only == True, force audio-shuffle every
                        # step (no text-shuffle, no video-shuffle). This regulariser
                        # specifically targets the audio shortcut and avoids the
                        # text-anchor + AV regression observed with the full cycle
                        # (cf. supp Tab. ltbreakdown).
                        if getattr(self, 'hn_audio_only', False):
                            neg_type_idx = 1
                        else:
                            neg_type_idx = neg_step % 3

                        with torch.no_grad():
                            if neg_type_idx == 0 and neg_text_input_ids is not None:
                                neg_embed = self._forward_submodal_embedding(
                                    neg_text_input_ids, _cached_audio_features, _cached_video_embeds,
                                    video_grid_thw, audio_feature_lengths, video_second_per_grid,
                                    has_audio=True, has_video=True, output_hidden_states=output_hidden_states,
                                )
                                hard_neg_embeds.append(neg_embed)
                            elif neg_type_idx == 1 and _cached_audio_features is not None and batch_size_local > 1:
                                audio_counts = (input_ids == self.config.audio_token_id).sum(dim=1)
                                if audio_counts.unique().numel() == 1:
                                    n_a = audio_counts[0].item()
                                    a_splits = list(_cached_audio_features.split(n_a))
                                    perm_a = torch.randperm(batch_size_local, device=input_ids.device)
                                    while (perm_a == torch.arange(batch_size_local, device=input_ids.device)).any():
                                        perm_a = torch.randperm(batch_size_local, device=input_ids.device)
                                    shuffled_audio = torch.cat([a_splits[p] for p in perm_a])
                                    neg_embed = self._forward_submodal_embedding(
                                        input_ids, shuffled_audio, _cached_video_embeds,
                                        video_grid_thw, audio_feature_lengths, video_second_per_grid,
                                        has_audio=True, has_video=True, output_hidden_states=output_hidden_states,
                                    )
                                    hard_neg_embeds.append(neg_embed)
                            elif neg_type_idx == 2 and _cached_video_embeds is not None and batch_size_local > 1:
                                video_counts = (input_ids == self.config.video_token_id).sum(dim=1)
                                if video_counts.unique().numel() == 1 and video_grid_thw is not None and (video_grid_thw == video_grid_thw[0:1]).all():
                                    n_v = video_counts[0].item()
                                    v_splits = list(_cached_video_embeds.split(n_v))
                                    perm_v = torch.randperm(batch_size_local, device=input_ids.device)
                                    while (perm_v == torch.arange(batch_size_local, device=input_ids.device)).any():
                                        perm_v = torch.randperm(batch_size_local, device=input_ids.device)
                                    shuffled_video = torch.cat([v_splits[p] for p in perm_v])
                                    neg_embed = self._forward_submodal_embedding(
                                        input_ids, _cached_audio_features, shuffled_video,
                                        video_grid_thw, audio_feature_lengths, video_second_per_grid,
                                        has_audio=True, has_video=True, output_hidden_states=output_hidden_states,
                                    )
                                    hard_neg_embeds.append(neg_embed)
                    else:
                        # No augmented data available — use anchor as pos (loss ≈ 0)
                        pos_embed = anchor_embed
                        hard_neg_embeds = []

                    # Obj1 loss: all ranks execute exactly 1 all_gather call here
                    loss_obj1 = self._compute_infonce_with_hard_negatives(
                        anchor_embed, pos_embed, hard_neg_embeds, self.sim_temperature)

                    # --- Objective 2 & 3: Single-modal alignment ---
                    # Anchor detached: sub-modal learns to align, anchor treated as fixed target
                    # Deterministic selection using global_step to keep all ranks in sync
                    # (random selection can cause different all_gather counts → NCCL deadlock)
                    anchor_detached = anchor_embed.detach()
                    loss_obj2 = torch.tensor(0.0, device=anchor_embed.device)
                    loss_obj3 = torch.tensor(0.0, device=anchor_embed.device)

                    # Always compute all 3 single-modal embeddings if available,
                    # then deterministically pick 2 for Obj2+Obj3
                    text_embed_single = None
                    audio_embed_single = None
                    video_embed_single = None

                    if label_ids is not None:
                        text_input_embeds = self.get_input_embeddings()(label_ids)
                        text_attn = label_attention_mask if label_attention_mask is not None else label_ids.ne(PAD_TOKEN_ID).to(torch.int64)
                        text_outputs = self.model(
                            inputs_embeds=text_input_embeds, attention_mask=text_attn,
                            output_hidden_states=output_hidden_states, return_dict=True,
                        )
                        text_embed_single = text_outputs[0][:, -1, :]  # Layer N only, no Fusion

                    if audio_only_input_ids is not None:
                        audio_embed_single = self._forward_submodal_embedding(
                            audio_only_input_ids, _cached_audio_features, _cached_video_embeds,
                            video_grid_thw, audio_feature_lengths, video_second_per_grid,
                            has_audio=True, has_video=False, output_hidden_states=output_hidden_states,
                        )

                    if video_only_input_ids is not None:
                        video_embed_single = self._forward_submodal_embedding(
                            video_only_input_ids, _cached_audio_features, _cached_video_embeds,
                            video_grid_thw, audio_feature_lengths, video_second_per_grid,
                            has_audio=False, has_video=True, output_hidden_states=output_hidden_states,
                        )

                    # Use anchor as fallback for missing modalities so that all ranks
                    # always execute exactly 3 all_gather calls (2 for Obj2 + 1 for Obj3)
                    embeds_all = [
                        text_embed_single if text_embed_single is not None else anchor_detached,
                        audio_embed_single if audio_embed_single is not None else anchor_detached,
                        video_embed_single if video_embed_single is not None else anchor_detached,
                    ]  # [text=0, audio=1, video=2]

                    # Deterministic pair rotation synced across all ranks
                    step_idx = self.cache_dict.get('_tuple_step', 0)
                    self.cache_dict['_tuple_step'] = step_idx + 1
                    pair_choices = [(0, 1), (0, 2), (1, 2)]  # (T,A), (T,V), (A,V)
                    pair = pair_choices[step_idx % 3]
                    embed_a = embeds_all[pair[0]]
                    embed_b = embeds_all[pair[1]]

                    # Obj2: both single-modal → full-modal alignment (anchor detached)
                    # Fixed 2 all_gather calls
                    loss_obj2 = (self._compute_symmetric_infonce(anchor_detached, embed_a, self.sim_temperature)
                               + self._compute_symmetric_infonce(anchor_detached, embed_b, self.sim_temperature)) / 2

                    # Obj3: pairwise alignment between the two single modalities
                    # Fixed 1 all_gather call
                    loss_obj3 = self._compute_symmetric_infonce(embed_a, embed_b, self.sim_temperature)

                    # --- Total TupleInfoNCE loss ---
                    loss = (self.tuple_aug_weight * loss_obj1
                            + self.tuple_single_weight * loss_obj2
                            + self.tuple_bi_weight * loss_obj3)

                    return ModelOutput(
                        loss=loss,
                        v2t_loss=loss_obj1,
                        t2v_loss=loss_obj2 + loss_obj3,
                        tuple_aug_loss=loss_obj1,
                        tuple_single_loss=loss_obj2,
                        tuple_bi_loss=loss_obj3,
                    )
